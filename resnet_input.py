...
class TraningSet(object):
    ...
    def __init__(self):
        ...
        label_path = '/home/long/image_scene_training/list.csv'//根据你自己的label路径设置
        self.all_data = read_csvlist(label_path)
        random.shuffle(self.all_data)
        self.start = 0
        self.end = 0
        self.dataset_dir = '/home/long/image_scene_training'

    def next_batch(self, batch_size, img_side_length):  //生成image，label的batch
...
def write_csvlist(listfile, data, header = [])://测试时类别写入的路径
...
def read_csvlist(listfile)://读取label
...
def img_resize(filepath, side_length)://对图片进行的裁剪操作，图片源像素应大于224*224
